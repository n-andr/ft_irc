...
if command -v gnome-terminal >/dev/null 2>&1; then
  ...
  echo "Choosing terminal"
  LAUNCH=$'osascript -e \'tell app "Terminal"\' -e  \'do script'
  LAUNCH_END=$'\' -e \'end tell\''
...
launch_client() {
  ...
  ENV_VARS='CLIENT_NAME='$name' NICK='$nick' REAL='$real' HOST='$HOST' PORT='$PORT' PASS='$PASS' CLIENT_PATH=\"'`pwd`'/launch-client.sh\"'
#   CLIENT_CMD='
#     set -euo pipefail
#     exec 3>/dev/tty
...
#   '
  ...
  echo "$LAUNCH \"export $ENV_VARS; bash -lc \$\\\"CLIENT_PWD\\\"\" $LAUNCH_END"
  eval "$LAUNCH \"export $ENV_VARS; bash \$CLIENT_PATH\" $LAUNCH_END"
}
